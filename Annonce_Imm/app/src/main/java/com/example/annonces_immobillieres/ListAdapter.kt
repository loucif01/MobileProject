...
class ListAdapter(private var list: List<Announce>) : RecyclerView.Adapter<MovieViewHolder>() {
    ...
    fun filter(keyWords:String?){
        ...
        if(keyWords!!.isEmpty()){
            this.list = Announces.instance.listAnnonces
        }else{
            this.list = Announces.instance.listAnnonces.filter {
                    announce -> announce.description.contains(keyWords,true)
                    ||  announce.wilaya.contains(keyWords,true)
                    ||  announce.username.contains(keyWords,true)
                    ||  announce.mobile.contains(keyWords,true)
                    ||  announce.email.contains(keyWords,true)
                    ||  announce.type.contains(keyWords,true)
